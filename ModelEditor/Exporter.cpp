					...
					for (int k = 0; k < obj->materialCount; k++)
					{
						...
						for (int l = 0; l < obj->materialBlockCount; l++)
						{
							if (obj->materialBlocks[l].materialID == k
								&& obj->materialBlocks[l].effect & XE_OPACITY)
							{
								m_transparentMaterials.push_back(&obj->materials[k]);
							}
						}
					}
				}
...
void CDAEExporter::_writeEffects()
{
	...
	for (auto it = m_materials.begin(); it != m_materials.end(); it++)
	{
		...
		// Apparemment 3Ds max ne le supporte pas, dommage...
		/*
		if (m_transparentMaterials.contains(it.value()))
		{
			...
			f.appendChild(m_doc.createTextNode("0"));
...
void CDAEExporter::_writeAnimations()
{
	...
	for (auto it = m_animations.begin(); it != m_animations.end(); it++)
	{
		...
		}
		{
			...
			for (int i = 0; i < m_frameCount; i++)
			{
				trans = D3DXVECTOR3(frames[i].pos.x, frames[i].pos.y, frames[i].pos.z);
				quat = D3DXQUATERNION(frames[i].rot.x, frames[i].rot.y, frames[i].rot.z, frames[i].rot.w);

				D3DXMatrixTranslation(&m1, trans.x, trans.y, trans.z);
				D3DXMatrixRotationQuaternion(&m2, &quat);
				m2 *= m1;
...
string CDAEExporter::_getMaterialID(Material* mat)
{
	for (auto it = m_materials.begin(); it != m_materials.end(); it++)
	{
		if (it.value() == mat)
...
{
	const D3DXMATRIX tempMat(mat._11, mat._12, -mat._13, mat._14,
